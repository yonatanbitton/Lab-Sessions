...
void fileDisplay(){
	...
	printf("cursor on file: %x\n",cursor);
	printf("location: %x\n",location);
	int readFromSys = read(fd,buf,size*length);			/*read from fileName size*length units*/
	close(fd);
	printf("PrintHexa\n");								/*printing what we read from the fileName*/
...
void loadToMem(){
	...
	int fd;
 	if (fileName==NULL) {
  	...
  	printf("Please enter location and length\n");
 	char* locLenBuf = malloc(1000); 
	fgets (locLenBuf,1000,stdin);
	sscanf(locLenBuf,"%x %d",&location,&length);
	length=length*size;
	...
	if (debugFlag==1)
		printf("Debug: location and length are: %x %d\n", location, length);
	dataPtr = (char*) malloc (length);
	if (debugFlag==1)
		printf("Debug: fileName is: %s, dataPtr: %x, location: %x, length: %d \n",fileName, dataPtr, location, length);

	int cursor=lseek(fd,location,SEEK_SET);
	int returnVal = read(fd,dataPtr,length); 		/* 101 - ma sherashum bsection.  */
	printf("Loaded 10 bytes into %02p\n",dataPtr);
	close(fd);
	free (locLenBuf);
	/*printf("The hexadecimal read units are:");
	PrintHex(dataPtr,length);*/	
}
